<?php
trait ModifiableMySqlQueryTrait
{
	...
	{
		sqlite_query("DELETE FROM $tableName" . ((isset($condition)) ? " WHERE $condition" : ''));
	}

	public static function deleteById(string $tableName, int $id, string $idColName = 'id'): void
	{
		sqlite_query("DELETE FROM $tableName WHERE $idColName = $id");
	}
	...
	public static function insertSet(string $tableName, array $assoc, bool $whenDuplicateKeyUpdate = true): void
	{
		...
		sqlite_query("INSERT $tableName SET $assignmentList" . (($whenDuplicateKeyUpdate) ? " ON DUPLICATE KEY UPDATE $assignmentList" : ''));
	}

	public static function insertVals(string $tableName, array $colNames, array $vals, bool $whenDuplicateKeyUpdate = true): void
	{
		sqlite_query("INSERT $tableName " . self::getBracketColNameList($colNames) . " VALUES " . self::getBracketValLists($vals) . (($whenDuplicateKeyUpdate) ? ' ON DUPLICATE KEY UPDATE ' . self::getAssignmentListWhenDuplicateKeyUpdate($colNames) : ''));
	}

	public static function update(string $tableName, array $assoc, ?string $condition = null): void
	{
		sqlite_query("UPDATE $tableName SET " . self::getAssignmentList($assoc) . ((isset($condition)) ? " WHERE $condition" : ''));
	}

	public static function updateById(string $tableName, array $assoc, int $id, string $idColName = 'id'): void
	{
		sqlite_query("UPDATE $tableName SET " . self::getAssignmentList($assoc) . " WHERE $idColName = $id");
	}
}
